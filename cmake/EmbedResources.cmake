function (embed_resources)
    ...
    cmake_parse_arguments (
        ${PROJECT_CMAKE_NAMESPACE} "" "${oneValueArgs}" "${multiValueArgs}" ${ARGN}
    )
    ...
    list (LENGTH ${PROJECT_CMAKE_NAMESPACE}_RESOURCES ${PROJECT_CMAKE_NAMESPACE}_RESOURCES_COUNT)
    set (counter 1)
    foreach (resource IN LISTS ${PROJECT_CMAKE_NAMESPACE}_RESOURCES)
        if (NOT (${resource} STREQUAL "TEXT"))
            if (counter LESS ${PROJECT_CMAKE_NAMESPACE}_RESOURCES_COUNT)
                list (GET ${PROJECT_CMAKE_NAMESPACE}_RESOURCES ${counter} next_value)
    ...
    add_library (
        ${${PROJECT_CMAKE_NAMESPACE}_TARGET}_resources STATIC
        ${RESOURCE_SOURCES}
    )
    set_property (TARGET ${${PROJECT_CMAKE_NAMESPACE}_TARGET}_resources PROPERTY POSITION_INDEPENDENT_CODE ON)

    target_include_directories (
        ${${PROJECT_CMAKE_NAMESPACE}_TARGET}_resources SYSTEM INTERFACE
        ${CMAKE_CURRENT_BINARY_DIR}/resources/
    )

    target_link_libraries (
        ${${PROJECT_CMAKE_NAMESPACE}_TARGET} PUBLIC
        ${${PROJECT_CMAKE_NAMESPACE}_TARGET}_resources
    )
endfunction ()
